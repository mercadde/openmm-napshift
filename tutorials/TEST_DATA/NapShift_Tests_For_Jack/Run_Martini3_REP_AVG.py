...
def create_replicate(grofile, topfile, simulation_outdir,
                     temperature=298*unit.kelvin,pressure=1,timestep=10*unit.femtosecond, GPU=0,
    ...
    conf = GromacsGroFile(grofile)
    box_vectors = conf.getPeriodicBoxVectors()
    martini_topfile = martini.MartiniTopFile(topfile, periodicBoxVectors=box_vectors, epsilon_r=15)
    ...
    if use_NapShift:
        napshift_force = get_napshift_force(top, CS_file, model_type='martini')
    ...
    platform.loadPluginsFromDirectory('shares/mcul245/research/ressci201900070-rcha387/openmm-napshift/build_REP_AVG')
...
parser.add_argument('--NapShift_max_K', default=25, type=float)
...
for i in range(args.num_replicates):
    rep = create_replicate(args.grofile, args.topfile, f"{args.simulation_outdir}/rep{i}",
                           temperature=args.temp*unit.kelvin,pressure=args.pressure,timestep=timestep, 
